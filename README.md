...
From e general perspective, the scheme is made such that:
   - the server can generate a unique, secret token for a user
...
This scheme is basically a OTP. So as long as it's used once, we're fine. But in our attack model, the attacker gets a guest token. Then, it can send **other tokens**. Those tokens are decrypted using the same **random block**. THIS IS NOT SECURE!!!
